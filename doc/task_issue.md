...
| 2026-04-06 | 推送当前 `api-work` 修复并重启宝塔线上 `site-gateway` | 阻塞 | 当前提交已推到 `origin/main` 的 `67ad0d0`；但 `ssh root@api-work.usfan.net` 与 `ssh root@38.246.250.228` 都卡在密码认证，宝塔浏览器执行器也返回 `Transport closed`，所以还没法代你在线上执行重启 |
